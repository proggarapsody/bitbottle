...
func TestAdd_AddsReviewer(t *testing.T) {
	t.Parallel()
	...
	cmd := defaultreviewer.NewCmdAdd(f, nil)
...
func TestAdd_RequiresUserFlag(t *testing.T) {
	t.Parallel()
...
func TestAdd_RequiresTwoArgs(t *testing.T) {
	t.Parallel()
	f, _, _ := factorytest.New(t, factorytest.Opts{InitialConfig: cloudConfig})
	cmd := defaultreviewer.NewCmdAdd(f, nil)
	cmd.SetArgs([]string{"myws", "--user", "abc123"})
	err := cmd.Execute()
	require.Error(t, err)
}

func TestAdd_ServerBackend_ReturnsUnsupportedError(t *testing.T) {
	t.Parallel()
