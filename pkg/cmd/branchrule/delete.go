...
// NewCmdDelete builds the `branch-rule delete` cobra command.
func NewCmdDelete(f *factory.Factory) *cobra.Command {
	var hostname string
	cmd := &cobra.Command{
		Use:   "delete [PROJECT/REPO] ID",
		Short: "Delete a branch restriction rule from a repository",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := br.DeleteBranchRule(ref.Project, ref.Slug, id); err != nil {
				return err
			}
			fmt.Fprintf(f.IOStreams.Out, "Branch rule %d deleted\n", id)
			return nil
		},
	}
