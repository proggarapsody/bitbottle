...
// list_tree, and put_file — onto the MCP server.
func registerSourceTools(s *mcpserver.MCPServer, h *handlers) {
	s.AddTool(
		mcplib.NewTool("put_file",
			mcplib.WithDescription("Create or update a file on a branch. Content is specified inline via the 'content' parameter. Both Bitbucket Server and Cloud are supported."),
			...
			mcplib.WithString("path", mcplib.Required(), mcplib.Description("Repo-relative file path to create or update")),
			mcplib.WithString("branch", mcplib.Required(), mcplib.Description("Target branch name")),
			mcplib.WithString("message", mcplib.Required(), mcplib.Description("Commit message")),
			mcplib.WithString("content", mcplib.Required(), mcplib.Description("File content as a string")),
			mcplib.WithString("source_commit", mcplib.Description("Expected HEAD SHA for conflict detection (optional)")),
...
func (h *handlers) putFile(_ context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	hostname := req.GetString("hostname", "")
	project, err := requireString(req, "project")
	if err != nil {
		return errResultErr(err), nil
	...
	sourceCommit := req.GetString("source_commit", "")

	client, err := h.resolveBackend(hostname)
	if err != nil {
		return errResultErr(err), nil
	}
	sw, err := backend.AsSourceWriter(client, hostname)
	if err != nil {
		return errResultErr(err), nil
	}
	if err := sw.PutFile(project, slug, path, backend.PutFileInput{
		Content:      content,
		Branch:       branch,
		Message:      message,
...
// Compile-time guard for SourceWriter.
var _ backend.SourceWriter = (backend.SourceWriter)(nil)
