...
func toIssueUserDomain(w *cloudgen.CloudIssueUser) backend.User {
	if w == nil {
		return backend.User{}
	}
...
func toIssueDomain(w cloudgen.CloudIssue) backend.Issue {
	issue := backend.Issue{
		ID:       w.ID,
		Title:    w.Title,
		State:    w.State,
		Kind:     w.Kind,
		Priority: w.Priority,
		Reporter: toIssueUserDomain(w.Reporter),
		WebURL:   w.Links.HTML.Href,
		Content:  w.Content.Raw,
	}
	if w.Assignee != nil {
		u := toIssueUserDomain(w.Assignee)
		issue.Assignee = &u
	}
...
func (c *Client) ListIssues(ns, slug, state string, limit int) ([]backend.Issue, error) {
	...
	return paging.Collect(c.http, path, func(body []byte) ([]backend.Issue, error) {
		var page cloudPagedResponse[cloudgen.CloudIssue]
		if err := json.Unmarshal(body, &page); err != nil {
			return nil, err
		}
		out := make([]backend.Issue, 0, len(page.Values))
		for _, w := range page.Values {
			out = append(out, toIssueDomain(w))
		}
		return out, nil
	}, limit)
}
...
func toIssueCommentDomain(w cloudgen.CloudIssueComment) backend.IssueComment {
	c := backend.IssueComment{
		ID:      w.ID,
		Content: w.Content.Raw,
	}
	if w.Author != nil {
		c.Author = toIssueUserDomain(w.Author)
	}
	if t, err := time.Parse(time.RFC3339, w.CreatedOn); err == nil {
		c.CreatedOn = t
	}
...
func (c *Client) ListIssueComments(ns, slug string, id int) ([]backend.IssueComment, error) {
	...
	return paging.Collect(c.http, path, func(body []byte) ([]backend.IssueComment, error) {
		var page cloudPagedResponse[cloudgen.CloudIssueComment]
		if err := json.Unmarshal(body, &page); err != nil {
			return nil, err
		}
		out := make([]backend.IssueComment, 0, len(page.Values))
		for _, w := range page.Values {
			out = append(out, toIssueCommentDomain(w))
		}
		return out, nil
	}, 0)
}
