...
| `admin rate-limit get` | ✅ | Show rate-limiting configuration — scope **ADMIN-RATE-LIMIT** |
| `admin rate-limit set` | ✅ | Update rate-limiting configuration — scope **ADMIN-RATE-LIMIT** |
...
|---|---|---|---|---|---|
| **TESTSCRIPT** | **Whole-binary script tests (testscript)** — `test/script/` harness + `bb-fake` server/cloud testscript command + seed corpus of `.txtar` scripts covering high-value end-to-end flows (`auth status`, `repo list`/Cloud+Server, `pr list`/Cloud+Server, `pr view`, errfmt catalogue, `bitbottle.host` defaulting). Hermetic by default; opt-in real Bitbucket via `BITBOTTLE_E2E=1`. Issue [#399](https://github.com/proggarapsody/bitbottle/issues/399). **P0 — testing-tier 3.** | N/A | DX | ✅ |
| **RELEASE-DRY-RUN** | **PR-gated release pipeline dry-run** — new `.github/workflows/release-dryrun.yml` triggered when a PR touches `.goreleaser.yaml`, `.github/workflows/release.yml`, `.github/workflows/scorecard.yml`, or `packages/mcp-npm/**`. Runs `goreleaser release --snapshot --clean` and `npm publish --dry-run --provenance` in the same matrix as the real release. Would have blocked [#384](https://github.com/proggarapsody/bitbottle/pull/384)/[#388](https://github.com/proggarapsody/bitbottle/pull/388)/[#391](https://github.com/proggarapsody/bitbottle/pull/391) at PR review (cycle 93's 75-min cascade → 0 min). **P0 — testing-tier 5.** | N/A | DX | ✅ |
| **HINT-FLAG-CONTRACT** | **Hint↔flag cross-file contract test** — new `pkg/errfmt/contract_test.go` that walks every catalogue entry's `hints[]`, extracts every `--flag` / `` `-f` `` token, and asserts each is registered as a persistent/root/subcommand flag reachable from the command that surfaces the matching `ErrorCode`. Five-line fixture loop. Statically catches [#387](https://github.com/proggarapsody/bitbottle/pull/387) (`-k` promised, not wired on third command) and [#394](https://github.com/proggarapsody/bitbottle/pull/394) (phantom `--debug`) and every future hint drift. **P0 — testing-tier 4.** | N/A | DX | ✅ |
| **TESTSCRIPT-BACKFILL** | **Txtar scripts for commands shipped without tier-3 coverage** — ARCHITECTURE.md §"Test tiers" requires every new user-visible command to have at least one `.txtar` script. Seven features that shipped after cycle 101 (TESTSCRIPT) are missing theirs: `pr unready`, `pr edit --remove-reviewer`, `pr checks --json`, `pr reviewer-group list/add/remove`, `pr suggestion apply`, `auth doctor`, `auth login --self-signed` (TLS trust flow). Backfill one `.txtar` per command above (8 scripts total). Additionally, wire `scripts/tdd-check.sh` (or a new `auto-iter/scripts/preflight.sh` check) to fail when a `feat(<noun>):` commit touches `pkg/cmd/<noun>/` without a matching `test/script/testdata/<noun>_*.txtar` — closing the gate that silently failed 7 times. **P1 — testing-tier 3 enforcement.** | N/A | DX | ✅ |
| **UPGRADE-PATH-TESTS** | **Pre-migration state `.txtar` sub-corpus** — once TESTSCRIPT lands, add `test/script/testdata/upgrade/*.txtar` that drops a pre-migration on-disk shape (token-in-`hosts.yml`, pre-normalization hostname, pre-keyring-key-rename, deprecated `pipeline variable` config) into hermetic HOME, runs `bitbottle auth status` (or whichever flow triggers the upgrade), and asserts the resulting on-disk shape + observable behaviour. Pins the [`08b3d9a`](https://github.com/proggarapsody/bitbottle/commit/08b3d9a) post-migrate auth class of bugs. **P0 — testing-tier 3 sub-corpus (depends on TESTSCRIPT).** | N/A | DX | ✅ |
...
| REPO-LABELS | **Cloud Issue Labels (Components)** | `repo label list/add/delete [PROJECT/REPO]`. Cloud: `GET/POST/DELETE /repositories/{ws}/{slug}/components`. Server → typed `host.unsupported`. New optional interface `RepoLabelClient`. MCP triplet `list_repo_labels`, `add_repo_label`, `delete_repo_label`. Enables agents to read/manage valid component set for `issue create`/`issue edit`. — scope **REPO-LABELS** | Cloud | 2 | ✅ |
...
| PR-CHECKS-JSON | **JSON output for `pr checks`** | `pr checks PR_ID` lists CI/build statuses as a text table only. Add structured JSON/YAML output so agents and scripts can poll programmatically: `pr checks 42 --json` emits `[{key,state,name,description,url},…]`. `--json` and `--watch` are mutually exclusive (watch is interactive-TTY only; error if both set). Implementation: `checksFields` printer in `checks.go`, two new tests. | Both | 2 | ✅ |
| CMD-SUBPKG-PR-LIFECYCLE | **Migrate PR lifecycle commands to subpackages** | The `pr/` package has 84 files in a flat layout. As a first increment of CMD-SUBPKG for `pr/`, migrate the small lifecycle-only commands — `approve.go`, `unapprove.go`, `decline.go`, `reopen.go`, `ready.go`, `unready.go` — to `pkg/cmd/pr/{approve,unapprove,decline,reopen,ready,unready}/` subpackages matching the `pipeline/cache/` pattern. Each is a 1-2 file command with a thin implementation. One PR per group is the intention; this is group 1. | Both | DX | ✅ |
| PR-ACTIVITY-JSON | **JSON output for `pr activity`** | `pr activity PR_ID` currently prints a human-readable timeline of PR events (comments, approvals, status changes, commit pushes). Add `--json` output so agents can programmatically consume the event log: each event gets a JSON object with `type`, `author`, `timestamp`, and `body` fields. Currently the MCP `get_pr_activity` tool already returns structured data; this brings the CLI to parity. Implementation: add `activityFields` printer, one new test. | Both | 2 | ✅ |
| REPO-DEFAULT-BRANCH | **Set repository default branch** | `bitbottle repo set-default-branch BRANCH [PROJECT/REPO]` — update the default/main branch of a repository. Cloud: `PUT /repositories/{ws}/{slug}` with `{"mainbranch": {"name": "branch"}}`. Server/DC: `PUT /rest/api/1.0/projects/{project}/repos/{slug}` with `{"defaultBranch": "branch"}`. Useful when migrating repos from `master` → `main` programmatically. MCP tool `set_default_branch` as companion. | Both | 3 | ✅ |
| WIRE-MIGRATE-PIPELINE-SCHEDULES | **Migrate pipeline_schedules adapter to gen/ types** | As part of WIRE-MIGRATE-REST, the Cloud `api/cloud/pipeline_schedules.go` adapter still uses hand-written `wirePipelineSchedule` / `wirePipelineScheduleInput` structs. Pattern: (1) add `PipelineSchedule` and `PipelineScheduleInput` to `gen/openapi.yaml`, (2) run `make gen`, (3) delete the wireXxx structs, (4) update the mapping functions to accept `gen.PipelineSchedule`. Existing adapter tests must pass unchanged. | Cloud | DX | ✅ |
| JSON-GOLDEN-COVERAGE | **Add `--json` golden-file tests to remaining commands** | The `json_golden_test.go` pattern exists for `commit`, `pipeline/list`, `pipeline/steps`, `webhook/list`, `branch`, `deploykey`, `branchrule/list`, but 7 command groups that expose `--json` output lack golden tests: `branchrule`, `context`, `issue`, `repo`, `search`, `sshkey`, `tag`. Each gets a `json_golden_test.go` using the existing `testhelpers.AssertGolden` pattern. Fixes: (1) catch unintended field renames that break scripted consumers, (2) the JSON-WHITELIST just shipped — golden files pin the default (all-fields) contract so `--json=field` tests have a reference baseline. One PR covering all 7 command groups. | N/A | DX | ✅ |
| CMD-SUBPKG-REPO | **Migrate `pkg/cmd/repo/` flat files to subpackages** | `pkg/cmd/repo/` has 17 flat `.go` files. As the next increment of the CMD-SUBPKG pattern (after CMD-SUBPKG-PR-LIFECYCLE migrated `pr/` lifecycle verbs), migrate all repo subcommands to `pkg/cmd/repo/{clone,create,delete,edit,fork,list,rename,set-default-branch,sync,transfer,view}/` subpackages. Each subpackage gets `<verb>.go` + `<verb>_test.go`, constructor `NewCmd<Verb>`, package name = verb. Update `pkg/cmd/repo/repo.go` to import subpackages via aliases. Lint, test suite must pass unchanged. | N/A | DX | ✅ |
| CLOUD-ADAPTER-TEST-COVERAGE | **Add unit tests to 8 untested Cloud adapter files** | `api/cloud/` has 8 adapter files with no co-located `*_test.go`: `admin.go`, `deployments.go`, `pipeline_logs.go`, `repo_default_branch.go`, `repo_forks.go`, `repo_transfer.go`, `repo_watchers.go`, `workspace_variables.go`. Each gets 2–3 table-driven tests using `httptest.NewTLSServer` covering: (a) correct path construction, (b) correct request body (for mutating methods), (c) domain-type mapping from response. Pattern: mirror `api/cloud/commit_test.go`. Raises overall adapter coverage and protects these operations from silent regressions. | Cloud | DX | ✅ |
...
| WIRE-MIGRATE-REST | **Complete wireXxx → gen migration for all remaining adapter files** | OPENAPI-TYPES (v1.72.0) introduced `api/cloud/gen/` and `api/server/gen/` and migrated the server PR adapter. ~65 adapter files still use hand-written `wireXxx` structs: all of `api/cloud/` (branches, commit, commit_comment, commit_status, default_reviewers_mgmt, deploy_keys, deployments, diff, issues, pipeline_caches, pipeline_schedules, pipeline_steps, pipeline_trigger, pipeline_variables, pipelines, pr_activity, pr_comments, pr_lifecycle, prs, repo_forks, repo_transfer, repo_watchers, repos, search, ssh_keys, workspace_members, webhooks) and the remaining `api/server/` files (branch_protect, branches, codeinsights, commit, commit_comment, commit_status, deploy_keys, deployments, diff, pr_comments, pr_suggestions, reactions, repos, reviewer_groups, ssh_keys, webhooks, workspace). Pattern per file: (1) add missing types to `gen/openapi.yaml`, (2) run `make gen`, (3) delete `wireXxx` struct, (4) update mapping function to accept `gen.Xxx`. Existing adapter tests must pass unchanged — they test observable behavior, not struct shape. No new CLI commands; no SKILL.md update needed. | N/A | DX | ✅ |
| UX-FLAG-AUDIT | **Audit every command for flag/hint drift** | Manual functional sweep of every `bitbottle <cmd>` against (a) its own `-h` flag list, (b) the matching section in `references/*.md` / `skills/SKILL.md`, (c) every error hint in `pkg/errfmt/errfmt.go` that promises a flag. Triggered by a real user incident: `tls.cert.unknown_ca` hints `-k`, but `-k` is only wired on `auth login` (as `--skip-tls-verify`) and `profile create` (as `--skip-tls`); running it on `pr approve` fails with "unknown shorthand flag" (see TLS-VERIFY-GLOBAL for the concrete fix). Audit deliverable: `docs/manual-tests/shared/flag-audit.md` containing a table of (command, hinted-flag, actually-wired?, reference-doc-claims?, fix-link) — one row per finding. Run mode: TTY pass through every command's `-h`, then drive each error path in `errfmt.go` (test fixtures or real failures) to verify the hint resolves to a real flag. Open a follow-up issue/scope for each finding so fixes ship as small PRs rather than a mega-audit-PR. | N/A | DX | ✅ |
| DEBUG-TRANSPORT-FLAG | **Wire `--debug` flag for HTTP transport logging** | The `CodeTransportTimeout` error hint in `pkg/errfmt/errfmt.go` referenced `--debug` as a way to get transport details, but no such flag was registered (found by UX-FLAG-AUDIT). The stale hint was removed; this scope adds the real flag. Wire a persistent `--debug` flag on root that enables verbose HTTP request/response logging to stderr: method, URL, status, headers, latency. Implementation: add `DebugHTTP bool` to factory options; when set, wrap the HTTP transport in a `roundtripper` that logs each exchange to `ios.ErrOut`. Update the `CodeTransportTimeout` hint to say `` pass `--debug` for request details `` once the flag ships. Touches: `pkg/cmd/root/root.go` (persistent flag), `pkg/cmd/factory/factory.go` (inject debug transport), `pkg/errfmt/errfmt.go` (restore hint), `pkg/errfmt/errfmt_test.go`. | N/A | DX | ✅ |
| TLS-VERIFY-GLOBAL | **Persistent `-k` / `--skip-tls-verify` flag on root** | The `tls.cert.unknown_ca` hint at `pkg/errfmt/errfmt.go:89` says *"pass `-k` or set `skip_tls_verify: true`"* — but `-k` is only defined on `auth login` (as `--skip-tls-verify`) and `profile create` (as `--skip-tls`). Running `bitbottle pr approve N -R …` against a self-signed CA hits the hint, the user copy-pastes `-k`, and Cobra errors with "unknown shorthand flag: 'k'". Fix: wire a persistent root flag `-k`/`--skip-tls-verify` (matches `curl -k` and gh's `--insecure-skip-tls-verify`) that overrides `hostCfg.SkipTLSVerify` in `pkg/cmd/factory/factory.go` for the current invocation only — no config write. Touches: `pkg/cmd/root/root.go` (persistent flag), `pkg/cmd/factory/factory.go` (apply override before `newHTTPClient`), `pkg/errfmt/errfmt.go` (hint stays truthful), `pkg/errfmt/errfmt_test.go`, `references/auth.md` (document the override). First finding of UX-FLAG-AUDIT. | N/A | DX | ✅ |
...
| WORKSPACE-AUDIT | **Workspace audit log listing** | `workspace audit [WORKSPACE] [--action ACTION] [--from DATE] [--limit N] [--json]` — list workspace audit log events. Cloud: `GET /2.0/workspaces/{ws}/log/audit` (paginated, supports `action` and `date_from` query params). Cloud only — Server/DC returns typed `host.unsupported`. New optional interface `AuditClient`. `AllFeatureSpecs` count 52 → 53. MCP tool `list_workspace_audit_log`. — scope **WORKSPACE-AUDIT** | Cloud | 2 | ✅ |
| WORKSPACE-IPALLOWLIST | **Workspace IP allowlist management** | `workspace ipallowlist list [WORKSPACE] [--limit N] [--json]`, `workspace ipallowlist add [WORKSPACE] CIDR [--description TEXT] [--enabled]`, `workspace ipallowlist delete [WORKSPACE] UUID [--confirm]` — manage Bitbucket Cloud workspace IP allowlists. Cloud: `GET/POST/DELETE /2.0/workspaces/{ws}/ipallowlists`. Cloud only — Server/DC returns typed `host.unsupported`. New optional interface `IPAllowlistClient`. `AllFeatureSpecs` count 53 → 54. MCP triplet `list_workspace_ipallowlists`, `create_workspace_ipallowlist`, `delete_workspace_ipallowlist`. — scope **WORKSPACE-IPALLOWLIST** | Cloud | 2 | ✅ |
| PIPE-CONFIG | **Pipeline repo-level enable/disable** | `pipeline config get [PROJECT/REPO] [--json]`, `pipeline config enable [PROJECT/REPO]`, `pipeline config disable [PROJECT/REPO]` — read and toggle the per-repo Pipelines master switch (the same setting in the Bitbucket UI under Repository settings → Pipelines settings). Cloud: `GET /repositories/{ws}/{slug}/pipelines_config`, `PUT /repositories/{ws}/{slug}/pipelines_config` with `{"enabled": bool}`. Cloud only — Server/DC returns typed `host.unsupported`. New optional interface `PipelineConfigClient` with `GetPipelineConfig`, `UpdatePipelineConfig`. Follows single-read + write-op pattern. MCP triplet `get_pipeline_config`, `enable_pipelines`, `disable_pipelines`. gh-parity gap (analogue of `gh workflow enable/disable`). — scope **PIPE-CONFIG** | Cloud | 2 | ✅ |
| MILESTONES | **Cloud Issue Milestones** | `milestone list [PROJECT/REPO] [--limit N] [--json]`, `milestone view [PROJECT/REPO] ID [--json]` — read-only access to the Cloud issue-tracker's milestone collection (the field already exists on `Issue.Milestone` but is currently opaque to the CLI). Cloud: `GET /repositories/{ws}/{slug}/milestones` (paginated), `GET /repositories/{ws}/{slug}/milestones/{milestone_id}`. Cloud only — Server/DC issue tracker has no milestone concept; returns typed `host.unsupported`. New optional interface `MilestoneClient` with `ListMilestones` (via `paging.Collect[T]`) and `GetMilestone`. Domain type `Milestone{ID, Name, Repo, WebURL}`. MCP pair `list_milestones`, `view_milestone`. Pairs with the existing issue surface — agents can now translate `issue.milestone.id` to a human-readable name. — scope **MILESTONES** | Cloud | 3 | ✅ |
| ISSUE-VERSIONS | **Cloud Issue Versions** | `version list [PROJECT/REPO] [--limit N] [--json]`, `version view [PROJECT/REPO] ID [--json]`, `version create [PROJECT/REPO] --name NAME`, `version delete [PROJECT/REPO] ID [--confirm]` — manage the Cloud issue tracker's "Version" facet (separate from milestones; used by `issue.version`). Cloud: `GET /repositories/{ws}/{slug}/versions` (paginated), `GET/POST/DELETE /repositories/{ws}/{slug}/versions/{version_id}`. Cloud only — Server/DC returns typed `host.unsupported`. New optional interface `IssueVersionClient` with `ListIssueVersions` (`paging.Collect[T]`), `GetIssueVersion`, `CreateIssueVersion`, `DeleteIssueVersion`. Domain type `IssueVersion{ID, Name, Repo, WebURL}`. MCP quartet `list_issue_versions`, `view_issue_version`, `create_issue_version`, `delete_issue_version`. Completes the issue-faceting surface alongside REPO-LABELS (components) and MILESTONES. — scope **ISSUE-VERSIONS** | Cloud | 3 | ✅ |
| WORKSPACE-PROJECT-CRUD | **Cloud Workspace Project CRUD** | `workspace project create WORKSPACE --key KEY --name NAME [--description D] [--private]`, `workspace project edit WORKSPACE KEY [--name NAME] [--description D] [--private=BOOL]`, `workspace project delete WORKSPACE KEY [--confirm]`, `workspace project view WORKSPACE KEY [--json]` — extends the existing `project list WORKSPACE` (✅) to full CRUD. Cloud: `POST /workspaces/{ws}/projects`, `GET /workspaces/{ws}/projects/{key}`, `PUT /workspaces/{ws}/projects/{key}`, `DELETE /workspaces/{ws}/projects/{key}`. Cloud only — Server/DC project CRUD already shipped as PROJECT-CRUD ✅; this is the Cloud-side analogue. New optional interfaces `CloudProjectCreator`, `CloudProjectEditor`, `CloudProjectDeleter`, `CloudProjectViewer`. `CreateCloudProjectInput`/`EditCloudProjectInput` use `*string`/`*bool` for partial PUT semantics. MCP quartet `create_workspace_project`, `view_workspace_project`, `edit_workspace_project`, `delete_workspace_project`. — scope **WORKSPACE-PROJECT-CRUD** | Cloud | 3 | ✅ |
| REPO-MIRROR | **Server/DC Mirror Servers** | `mirror list [--json]`, `mirror view ID [--json]`, `mirror repo list MIRROR_ID [--limit N] [--json]` — read Bitbucket Server/DC Smart Mirror configuration. Server: `GET /rest/mirroring/latest/mirrorServers` (paginated), `GET /rest/mirroring/latest/mirrorServers/{mirrorId}`, `GET /rest/mirroring/latest/mirrorServers/{mirrorId}/repos` (paginated). Cloud → typed `host.unsupported` (no equivalent feature). New optional interface `MirrorClient` with `ListMirrorServers`, `GetMirrorServer`, `ListMirroredRepos` (all via `paging.Collect[T]`). Domain types `MirrorServer{ID, Name, BaseURL, Enabled}` and `MirroredRepo{Slug, MirrorID, LastSyncAt, Status}`. Read-only initially — Smart Mirror configuration writes are admin-only and orthogonal. MCP triplet `list_mirror_servers`, `view_mirror_server`, `list_mirrored_repos`. Adds DC observability for cluster admins / DR-planning automation. — scope **REPO-MIRROR** | Server/DC | 3 | ✅ |
| WORKSPACE-PERMS | **Cloud Workspace Member Permissions** | `workspace perms list WORKSPACE [--limit N] [--json]`, `workspace perms repo list WORKSPACE [--limit N] [--json]`, `workspace perms grant WORKSPACE --user USER --permission member\|collaborator\|owner`, `workspace perms revoke WORKSPACE --user USER [--confirm]` — manage Cloud workspace-level membership and per-repo effective permissions (distinct from the Server/DC `perms project`/`perms repo` ✅ commands, which are Server-only). Cloud: `GET /workspaces/{ws}/permissions` (paginated user-level grants), `GET /workspaces/{ws}/permissions/repositories` (effective per-repo), `PUT /workspaces/{ws}/permissions/members/{member}`, `DELETE /workspaces/{ws}/permissions/members/{member}`. Server/DC → typed `host.unsupported` (no workspace concept). New optional interface `WorkspacePermsClient` with `ListWorkspaceMemberPerms`, `ListWorkspaceRepoPerms` (both via `paging.Collect[T]`), `GrantWorkspacePerm`, `RevokeWorkspacePerm`. MCP quartet. Closes the Cloud counterpart to PERMS ✅. — scope **WORKSPACE-PERMS** | Cloud | 3 | ✅ |
| SSH-KEY-SERVER | **Server/DC SSH Key Management** | Extends the shipped `ssh-key list/add/delete` (currently Cloud-only via SSH-KEYS ✅) to Bitbucket Server/DC. Server: `GET/POST/DELETE /rest/ssh/1.0/keys` for the authenticated user. The existing `SSHKeyClient` interface gains a Server implementation; `Both` after merge. Body shape differs (`text` field instead of Cloud's `key`), so mapping lives in the adapter; CLI/MCP surface unchanged. Removes the existing Cloud-only restriction documented in the SSH-KEYS row. Follows write-op pattern + `paging.Collect[T]` for list. No new CLI subcommands; no new MCP tools. — scope **SSH-KEY-SERVER** | Server/DC | 2 | ✅ |
...
| DEPLOY-KEY-PERMISSION | **Cloud Deploy Key Write-Permission Flag** | `deploy-key add PROJECT/REPO --key "..." [--permission read\|read-write]` — adds `--permission` flag to the shipped DEPLOY-KEY CRUD (Cloud honors `read`/`read_write`; Server silently ignores). Extends `DeployKeyInput`, Cloud wire POST, MCP `add_deploy_key` optional param. Pattern: write-op flag extension. — scope **DEPLOY-KEY-PERMISSION** | Both | 1 | ✅ |
...
| DEPLOY-KEY-PERMISSION | **Cloud Deploy Key Write-Permission Flag** | `deploy-key add PROJECT/REPO --key "..." [--label "..."] [--permission read\|read-write]` — extends the shipped DEPLOY-KEY ✅ CRUD with a `--permission` flag Cloud supports (`read` / `read_write`) but the original scope omitted. Adds `Permission` field to `CreateDeployKeyInput`; Cloud wires it in POST body; Server silently ignores (Server deploy keys are always read-only). Extends existing `add_deploy_key` MCP tool schema with optional `permission` param. Pattern: write-op flag extension. Both backends (Cloud honors; Server ignores). — scope **DEPLOY-KEY-PERMISSION** | Both | 1 | 🔲 |
| REPO-PIPELINE-VAR-VIEW | **Cloud Repo Pipeline Variable View** | `variable view [PROJECT/REPO] KEY [--scope repository] [--json]` — adds the missing `view` subcommand to the `variable` tree (VAR ✅ ships list/set/delete but not get-by-key). Cloud: list-then-fetch UUID pattern: `GET /repositories/{ws}/{slug}/pipelines_config/variables/{uuid}`. Cloud only (Server pipeline variables have no UUIDs; list is sufficient). Adds `GetRepoVariable(ns, slug, key string) (PipelineVariable, error)` to the Cloud pipeline variables adapter. MCP tool `get_repo_pipeline_variable`. — scope **REPO-PIPELINE-VAR-VIEW** | Cloud | 1 | ✅ |
| REPO-SYNC | **Repository Sync (Cloud Fork Sync)** | `repo sync [PROJECT/REPO] [--branch BRANCH] [--json]` — synchronize a fork with its upstream (`gh repo sync` parity). Cloud: `POST /repositories/{ws}/{slug}/merge-upstream` with `{"branch": "main"}`. Server → typed `host.unsupported` (no fork-upstream concept). New optional interface `RepoSyncer` with `SyncRepo(ns, slug, branch string) (SyncResult, error)`. Pattern: write-op with typed errors (ErrConflict on diverged history). MCP tool `sync_repo`. — scope **REPO-SYNC** | Cloud | 2 | ✅ |
| ADMIN-RATE-LIMIT | **Server/DC Admin Rate Limit Config** | `admin rate-limit get [--hostname H] [--json]`, `admin rate-limit set [--hostname H] --enabled --requests-per-hour N --throttle-wait-ms N` — read/write the Bitbucket Server/DC REST API rate-limit settings. Server: `GET/PUT /rest/api/1.0/admin/rate-limiting`. Cloud → typed `host.unsupported`. Extends `AdminClient` with `GetRateLimitConfig() (RateLimitConfig, error)` and `SetRateLimitConfig(in RateLimitConfig) error`. Pattern: single-read + write-op. MCP pair `get_rate_limit_config`, `set_rate_limit_config`. — scope **ADMIN-RATE-LIMIT** | Server/DC | 2 | ✅ |
| COMMIT-SEARCH | **Commit Search by Message / Author / Date** | `commit search [PROJECT/REPO] [--query Q] [--author USER] [--since DATE] [--until DATE] [--limit N] [--json]` — search commits by message keyword, author, or date range. Server: `GET /rest/api/1.0/projects/{k}/repos/{s}/commits?author=slug&since=SHA&until=SHA` (paginated, `paging.Collect[T]`). Cloud: `GET /repositories/{ws}/{slug}/commits?q=message~"Q"+AND+...`. Both backends. New optional interface `CommitSearcher` with `SearchCommits(ns, slug string, opts CommitSearchOpts) ([]Commit, error)`. MCP tool `search_commits`. — scope **COMMIT-SEARCH** | Both | 2 | 🔲 |
| PR-LOCK | **Lock / Unlock PR Conversation** | `pr lock PR_ID [--reason resolved\|off-topic\|too-heated\|spam]`, `pr unlock PR_ID` — lock a PR to prevent new comments (`gh pr lock` parity). Server/DC 8.0+: `PUT /rest/api/1.0/projects/{k}/repos/{s}/pull-requests/{id}` with `{"locked": true/false}`. Cloud → typed `host.unsupported`. New optional interface `PRLocker` with `LockPR` and `UnlockPR`. Uses `SRVVER.AtLeast(8,0)` version gate. MCP pair `lock_pr`, `unlock_pr`. — scope **PR-LOCK** | Server/DC | 2 | 🔲 |
| ADMIN-PUSH-RULES | **Server/DC Push Rules** | `admin push-rules list [--json]`, `admin push-rules create --type TYPE --pattern PATTERN [--scope global\|project\|repo]`, `admin push-rules delete ID [--confirm]` — manage server-side push rejection rules (large files, binary files, secrets regex). Server: `GET/POST/DELETE /rest/reject-reason/latest/push-rules`. Cloud → typed `host.unsupported`. New optional interface `PushRuleClient` with `ListPushRules`, `CreatePushRule`, `DeletePushRule`. Pattern: `paging.Collect[T]` + write-op. MCP triplet. — scope **ADMIN-PUSH-RULES** | Server/DC | 3 | 🔲 |
| TESTSCRIPT-BACKFILL-2 | **Txtar backfill for post-TESTSCRIPT-BACKFILL scopes** | ~28 feature scopes shipped after TESTSCRIPT-BACKFILL ✅ without `.txtar` coverage (SNIPPETS, BRANCH-MODEL, PIPE-ARTIFACTS, GROUP-MGMT, REACT-PR, REACT-COMMIT, AUTOMERGE, ADMIN-MAIL, ADMIN-BANNER, DEPLOY-KEY, DEFAULT-REVIEWERS, SSH-KEYS, REPO-TRANSFER, BRANCH-RULE, PIPELINE-SCHEDULE, COMMIT-FILE, PR-COMMITS, PR-FILES, REPO-WATCHER, COMMIT-STATUS-REPORT, PR-PARTICIPANTS, WORKSPACE-MEMBERS, USER-VIEW, REPO-FORKS, REPO-VISIBILITY, PAT-MGMT, ISSUE-ATTACH-VOTE-WATCH, REPO-LABELS). Add one `.txtar` per scope (or group adjacent pairs). Re-closes the "every new command ships with a txtar" gate. DX-only; no API changes. | N/A | DX | 🔲 |
