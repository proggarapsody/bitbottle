...
| PR-TEMPLATE | **PR Description Templates** | `pr template list`, `pr template view [NAME]`, `pr template set [--default]` — manage PR description templates. Cloud: `/repositories/{ws}/{slug}/pullrequests/defaultReviwers` and template APIs. Server: templates stored locally or via custom fields. Cloud only initially with a graceful `UnsupportedOnHost` stub. | Cloud | 3 | 🔲 |
