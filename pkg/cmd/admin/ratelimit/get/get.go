...
// Options holds parsed flags for `admin rate-limit get`.
type Options struct {
	Hostname string
	JSON     bool
}

// NewCmdGet builds the `admin rate-limit get` cobra command.
...
func getRun(f *factory.Factory, opts *Options) error {
	...
	fmt.Fprintf(f.IOStreams.Out, "Enabled: %v\nRequestsPerHour: %d\nThrottleWaitMS: %d\n",
		cfg.Enabled, cfg.RequestsPerHour, cfg.ThrottleWaitMS)
	return nil
}
