...
| GOMOD-FIX | **Pin `go.mod` to released Go version** | `go.mod:3` declares `go 1.25.5` (unreleased); pin to the latest released minor (e.g. `1.23.x`) to avoid GOTOOLCHAIN auto-download surprises on contributor and CI machines. One-line change + CI re-run. | N/A | DX | 🔲 |
