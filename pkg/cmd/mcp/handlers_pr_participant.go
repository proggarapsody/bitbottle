...
func (h *handlers) updatePRParticipant(_ context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	accountID, err := requireString(req, "account_id")
	if err != nil {
		return errResultErr(err), nil
	}
	...
	updater, err := backend.AsPRParticipantUpdater(client, hostname)
	if err != nil {
		return errResultErr(err), nil
	}

	ns = strings.ToUpper(ns)
	p, err := updater.UpdatePRParticipant(ns, slug, prID, accountID, state)
	if err != nil {
		return errResultErr(err), nil
	}
	return jsonResult(p)
